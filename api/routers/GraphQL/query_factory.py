...
# --------------------------------------------------------------------------------
# def define_query(model, order_by, offset, limit):
#     query = select(model)
#     if order_by:
#         query = sorting.add_sorting_to_query(query, order_by, UserModel)
#     if offset:
#         query = query.offset(offset)
#     if limit:
#         query = query.limit(limit)
...
#                 relationship_model = mapper.relationships[field_part].mapper.class_
#                 base_query = base_query.join(relationship_model)
#                 current_model = relationship_model

#             if not hasattr(current_model, field_parts[-1]):
#                 raise Exception(f"Field {field_name} not found in model")
#             group_columns.append(getattr(current_model, field_parts[-1]))

#         grouped_query = base_query.with_only_columns(*group_columns).group_by(*group_columns)
#         results = db_session.exec(grouped_query).all()

#         grouped_results = []
#         for row in results:
#             row_values = [row] if len(group_columns) == 1 else row
#             keys_dict = {}
#             query = base_query.with_only_columns(UserModel)
#             for i, col_val in enumerate(row_values):
#                 query = query.where(group_columns[i] == col_val)
#                 keys_dict[group_by[i]] = col_val

#             grouped_results.append(
#                 UserGroupResult(keys=keys_dict, query_wrapper=QueryWrapper(query=query, model=UserModel))
#             )
...
#     def users(self,
#               info: strawberry.Info,
#               order_by: typing.Optional[typing.List[sorting.UserOrderBy]] = None,
#               offset: typing.Optional[int] = None,
#               limit: typing.Optional[int] = None
#         ) -> typing.List[User]:
#         db_session: Session = info.context['db_session']
#         query = define_query(UserModel, order_by, offset, limit)
...
#     def items(self,
#               info: strawberry.Info,
#               order_by: typing.Optional[typing.List[sorting.ItemOrderBy]] = None,
#               offset: typing.Optional[int] = None,
#               limit: typing.Optional[int] = None
#     ) -> list[Item]:
#         db_session: Session = info.context['db_session']
#         query = define_query(ItemModel, order_by, offset, limit)
